...
def init_fb_actions():
    ...
    def _get_head_p(sparql):
        ...
        if match:
            ...
            print("No variable in sparql!")
            return 

        # one hop
        sparql_qpin = (
            PREFIX_NS
            + f"SELECT DISTINCT (SAMPLE(?_h) as ?_h) ?_pin WHERE {{ ?_h ?_pin {variable} . FILTER (?_h != {variable}) "
            + sparql_body
        )
        ...
        sparql_qpin2 = (
            PREFIX_NS
            + f"SELECT DISTINCT (SAMPLE(?_h) as ?_h) ?_pin1 ?_pin2 WHERE {{ ?_h ?_pin1 ?_c . ?_c ?_pin2 {variable} . FILTER (?_h != {variable}) "
            + sparql_body
        )
    ...
    def _get_p_tail(sparql):
        ...
        sparql_qpout = PREFIX_NS + f"SELECT DISTINCT ?_pout (SAMPLE(?_t) as ?_t) WHERE {{ {variable} ?_pout ?_t . "

        # exclude ring: ?_t != "xxx"@en
        sparql_qpout += f"FILTER (?_t != {variable}) "
        ...
        for item in query_res_pout:
            ...
            if item["_pout"]["type"] == "uri" and fb.is_cvt_predicate(p):
                sparql_cvt_out = (
                    PREFIX_NS
                    + f"SELECT DISTINCT ?_pout2 (SAMPLE(?_t2) as ?_t2) WHERE {{ ?_cvt ?_pout2 ?_t2 . FILTER (?_t2 != {variable}) "
                    + f"{variable} ns:{p} ?_cvt . "
                    + sparql_body
                )

...
def get_sqlite_cursor():
    if not hasattr(thread_local, "sql_client_facc1"):
        cache_db_path = os.getenv(
            "FACC1_CACHE_DB_PATH",
            os.path.join(os.path.dirname(os.path.dirname(__file__)), "database", "cache_facc1", "name_to_mids.db")
        )
        assert os.path.exists(cache_db_path), f"Cache database not found at {cache_db_path}. Please set FACC1_CACHE_DB_PATH environment variable or place the database at the default location."
...
def add_not_exists_or_exists_filter(sparql_query):
    ...
    # lines = part2.split(" .")
    # 先清理空白和换行
    cleaned_part2 = re.sub(r'\s+', ' ', part2.strip())

    # 使用 ". " 分割 triple
    lines = [line.strip() for line in cleaned_part2.split(". ") if line.strip()]
    # add root.
    ...
    for line in lines:
        ...
        if o.startswith('?') and p.startswith('ns:'):
            head_var[o] = s
